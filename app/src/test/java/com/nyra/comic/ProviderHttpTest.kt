...
class ProviderHttpTest {
    ...
    fun openAiCompatSendsCorrectPayloadAndParsesContent() {
        ...
        assertEquals("NYRA AI Comic Translation", lastHeaders["x-title"])
    ...
    /**
     * Gambar rujukan: mosaik HARUS jadi gambar pertama. Kalau urutannya
     * terbalik, model membaca halaman utuh sebagai sumber ID dan tidak akan
     * menemukan satu pun nomor merah.
     */
    @Test
    fun geminiMengirimDuaGambarDenganMosaikDiDepan() {
        responseCode = 200
        responseBody = """{"candidates":[{"content":{"parts":[{"text":"{\"1\": \"halo\"}"}]}}]}"""

        val p = GeminiProvider("gkey", "gemini-flash-latest")
        val out = p.translateBase64ForTest(
            listOf("TU9TQUlL", "SEFMQU1BTg=="), "prompt",
            "http://127.0.0.1:$port/v1beta/models/x:generateContent"
        )
        assertNotNull(out)

        val parts = JSONObject(lastBody).getJSONArray("contents")
            .getJSONObject(0).getJSONArray("parts")
        assertEquals("dua gambar + satu teks", 3, parts.length())
        assertEquals("TU9TQUlL", parts.getJSONObject(0).getJSONObject("inline_data").getString("data"))
        assertEquals("SEFMQU1BTg==", parts.getJSONObject(1).getJSONObject("inline_data").getString("data"))
        assertEquals("prompt", parts.getJSONObject(2).getString("text"))
    }

    @Test
    fun geminiSatuGambarTetapSatuBagianGambar() {
        responseCode = 200
        responseBody = """{"candidates":[{"content":{"parts":[{"text":"{}"}]}}]}"""

        GeminiProvider("gkey", "gemini-flash-latest").translateBase64ForTest(
            "QUJD", "prompt", "http://127.0.0.1:$port/v1beta/models/x:generateContent"
        )
        val parts = JSONObject(lastBody).getJSONArray("contents")
            .getJSONObject(0).getJSONArray("parts")
        assertEquals("tanpa rujukan payload harus tetap seperti dulu", 2, parts.length())
    }

    @Test
    fun openAiCompatMengirimDuaGambarDenganMosaikDiDepan() {
        responseCode = 200
        responseBody = openAiOk("{\"1\": \"halo\"}")

        val p = OpenAICompatProvider("k", "gpt-4o", url(), "OpenAI", true, true)
        val out = p.translateBase64(listOf("TU9TQUlL", "SEFMQU1BTg=="), "prompt")
        assertNotNull(out)

        val content = JSONObject(lastBody).getJSONArray("messages")
            .getJSONObject(0).getJSONArray("content")
        assertEquals("dua gambar + satu teks", 3, content.length())
